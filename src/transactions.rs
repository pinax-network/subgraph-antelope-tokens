...
pub fn insert_transaction(params: &str, tables: &mut Tables, clock: &Clock, transaction: &TransactionTrace) {
    ...
    for db_op_ext in collapsed_db_ops.iter() {
        insert_db_op(params, tables, clock, &db_op_ext.db_op, transaction, db_op_ext.index)
    }
}
